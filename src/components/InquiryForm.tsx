...
export function InquiryForm({productName='',compact=false}:{productName?:string;compact?:boolean}){
  ...
  return <form className={`inquiry-form ${compact?'compact':''}`} action={`https://formsubmit.co/${site.email}`} method="POST">
    ...
    <div className="form-grid">
      ...
      <label>{t('form.market')}<select required name="destination-market" defaultValue=""><option value="" disabled>Select market</option><option>Europe</option><option>North America</option><option>Middle East</option><option>Other</option></select></label>
